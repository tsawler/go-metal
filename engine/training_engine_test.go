...
// TestMPSTrainingEngineHybrid tests hybrid engine creation
func DisabledTestMPSTrainingEngineHybrid(t *testing.T) {
	device, err := cgo_bridge.CreateMetalDevice()
	if err != nil {
		t.Skipf("Metal device not available for hybrid test: %v", err)
	}
	defer cgo_bridge.DestroyMetalDevice(device)
	...
	engine, err := NewMPSTrainingEngineHybrid(config, modelConfig)
	if err != nil {
		t.Fatalf("Failed to create hybrid training engine: %v", err)
	}
	// Note: Skip cleanup to avoid CGO double-free issues
	// defer engine.Cleanup()
	...
	if engine.isDynamic {
		t.Error("Hybrid engine should not be dynamic")
	}

	t.Log("✅ MPSTrainingEngine hybrid tests passed")
}
...
// TestTrainingEngineHybridExecution tests hybrid training execution
func DisabledTestTrainingEngineHybridExecution(t *testing.T) {
	device, err := cgo_bridge.CreateMetalDevice()
	if err != nil {
		t.Skipf("Metal device not available for hybrid execution test: %v", err)
	}
	defer cgo_bridge.DestroyMetalDevice(device)
	...
	// Test 1: Wrong number of weight tensors
	inputTensor, err := memory.NewTensor([]int{2, 3, 32, 32}, memory.Float32, memory.GPU)
	if err != nil {
		t.Fatalf("Failed to create input tensor: %v", err)
	}
	defer inputTensor.Release()
	...
	// Wrong number of weights (hybrid expects 4)
	wrongWeights := []*memory.Tensor{inputTensor} // Wrong count
	_, err = engine.ExecuteStepHybrid(inputTensor, labelTensor, wrongWeights)
	if err == nil {
		t.Error("Expected error for wrong weight count")
	}

	// Test 2: Create correct weight tensors for hybrid (4 tensors: FC1 weight, FC1 bias, FC2 weight, FC2 bias)
	weightShapes := [][]int{
		{512, 64},  // FC1 weights (after conv features)
		{64},       // FC1 bias
		{64, 2},    // FC2 weights
		{2},        // FC2 bias
	}
	...
	// Test 3: Valid hybrid execution
	loss, err := engine.ExecuteStepHybrid(inputTensor, labelTensor, weightTensors)
	if err != nil {
		t.Logf("Hybrid training step failed as expected (uninitialized conv weights): %v", err)
	} else {
		t.Logf("Hybrid training step succeeded: loss=%f", loss)
	}

	t.Log("✅ Training engine hybrid execution tests passed")
}
