...
func TestSetMaxPoolSizeForTier(t *testing.T) {
	SetMaxPoolSizeForTier(4096, 200)
	if calculateMaxPoolSize(1024) != 200 {
		t.Errorf("Expected 200 for <=4KB tier, got %d", calculateMaxPoolSize(1024))
	}
	ResetMaxPoolSizes()
	if calculateMaxPoolSize(1024) != 100 {
		t.Errorf("Expected reset to 100 for <=4KB tier, got %d", calculateMaxPoolSize(1024))
	}
}
